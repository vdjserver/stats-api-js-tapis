...
def testAPI(base_url, entry, auth, verbose, force):
    ...
    if entry.get('response_type'):
        if type(data_json).__name__ != entry['response_type']:
            entry['result'] = FAIL_STRING
            entry['result_message'] = 'incorrect response_type: ' + type(data_json).__name__
            return
    ...
    if entry.get('records') is not None:
        if data_json.get('Result') is None:
            entry['result'] = FAIL_STRING
            entry['result_message'] = 'Result entry not found.'
            return
        else:
            if len(data_json['Result']) != entry['records']:
                entry['result'] = FAIL_STRING
                entry['result_message'] = 'Number of records does not match expected: ' + str(len(data_json['Result'])) + ' != ' + str(entry['records'])
                return
